...
function getTransactions() {
  ...
  for (let i = 0; i < vals.length; i++) {
    const r = vals[i];
    if (r[0] === "" || r[4] === "") continue;
    // Col I (r[8]) is the ONLY source of goal-spend info: it holds the goal name for a
    // goal spend, or "" for a normal transaction. We deliberately do NOT fall back to col J
    // (r[9]) — col J is unused, and stray text there used to be misread as a goal name,
    // wrongly flagging normal transactions as goal spends.
    const colI     = r[8];
    const goalName = (typeof colI === "string" && colI !== "" && colI !== "TRUE" && colI !== "FALSE")
                     ? colI
                     : "";
    const fromGoal = goalName !== "" || colI === true || colI === "TRUE" || colI === 1;
    transactions.push({
      ...
      fromGoal: fromGoal,
      goalName: goalName,
      goalId: (typeof r[9] === "string" && r[9].indexOf("goal:") === 0)
                ? (Number(r[9].slice(5)) || null) : null,           // Col J: goalId for goal-spends
      splitId: (typeof r[9] === "string" && r[9].indexOf("goal:") === 0)
                ? "" : ((r[9] !== "" && r[9] != null) ? String(r[9]) : "")  // Col J: splitId otherwise
    });
  }
  return { transactions };
}
...
function updateTransaction(rowId, data) {
  ...
  sheet.getRange(targetRow, 1, 1, 10).setValues([[
    ...
    dateVal.getMonth() + 1, getWeekNumber(dateVal),
    fromGoal ? goalName : "",  // Col I: goal name if goal spend, else blank
    (data.splitId !== undefined ? data.splitId : (existingSplit || ""))  // Col J: split-group id
  ]]);
...
function cleanupColumnJ() {
  ...
  for (let i = 0; i < vals.length; i++) {
    const v = vals[i][0];
    // Preserve split-group ids — only clear other (legacy) stray text.
    if (v !== "" && v !== null
